...
async def publish(m,uid):
    ...
    exp_text='Tidak pernah' if not e.expires_at else e.expires_at.astimezone(timezone.utc).strftime('%d %b %Y %H:%M UTC')
    if public_domain:
