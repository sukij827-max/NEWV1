...
async def publish(m,uid):
    ...
    if missing:
        await m.answer('❌ Field wajib kosong: '+', '.join(missing))
        return False
...
async def retry_publish(m:Message,bot:Bot):
    uid=m.from_user.id
    if not await gate(m,bot): return
    st=STATE.get(uid)
    if not st or st.get('step')!='expiration' or 'expiration_days' not in st.get('data',{}):
        await m.answer('⚠️ Tidak ada Experience yang siap diulang.')
        return
    try:
        await publish(m,uid)
    except Exception:
        await m.answer('❌ Masih gagal membuat Experience. Data draft tetap aman; cek log Railway sebelum mencoba lagi.')

@router.message(Command('selesai'))
async def finish_media(m:Message,bot:Bot):
    uid=m.from_user.id
    if not await gate(m,bot): return
    st=STATE.get(uid)
    if not st or st.get('step')!='media':
        await m.answer('⚠️ Saat ini bot tidak berada di tahap Media.')
        return
    st['step']='expiration'
    await save_draft(uid,st['kind'],'expiration',json.dumps(st['data'],ensure_ascii=False))
    await ask(m,uid)
...
async def text(m:Message,bot:Bot):
    ...
    if step=='title': st['data']['title']=(m.text or '').strip()[:255]; st['step']='intro'; await ask(m,uid)
        ...
        default=default_expiration_days(st['kind'])
        if raw in {'default','d','template'}:
            days=default
        else:
            try:
                days=int(raw)
            except (TypeError, ValueError):
                await m.answer(f'❌ Masukkan angka 0, 7, 14, 30, 60, atau 90. Atau ketik <code>default</code> untuk {default} hari.')
                return
            if days not in {0,7,14,30,60,90}:
                await m.answer(f'❌ Durasi {days} hari tidak tersedia. Pilih: 0, 7, 14, 30, 60, 90, atau <code>default</code>.')
                return
        st['data']['expiration_days']=days
        # Persist the selected value before creating the Experience so a transient
        # database/network failure does not lose the creator's input.
        await save_draft(uid,st['kind'],'expiration',json.dumps(st['data'],ensure_ascii=False))
        try:
            await publish(m,uid)
        except Exception:
            # Keep the draft and state intact; the next message can retry instead
            # of silently returning to/repeating the expiration prompt.
            await m.answer('⚠️ Experience belum berhasil dibuat karena terjadi kesalahan sementara. Data kamu tetap tersimpan. Ketik <code>retry</code> untuk mencoba lagi atau /cancel untuk batal.')
        return
