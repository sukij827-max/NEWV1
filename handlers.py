...
async def publish(m,uid):
    st=STATE.get(uid)
    if not st:
        await m.answer('⚠️ Sesi pembuatan sudah berakhir. Silakan mulai Create Experience lagi.')
        return False
    d=st['data']; missing=[x for x in ('title','intro','body') if not d.get(x)]
    if missing:
        await m.answer('❌ Field wajib kosong: '+', '.join(missing)); return False
    ...
    # Only database creation is fatal. Optional share-button generation must
    # never turn a successfully-created Experience into a false error.
    try:
        ...
        await m.answer('⚠️ Experience belum berhasil dibuat karena terjadi kesalahan pada database. Data kamu tetap tersimpan. Cek log Railway untuk detail error.')
        return False
    ...
    text=(f'🎉 <b>Experience berhasil dibuat!</b>\n\n✨ {e.title}\n🆔 <code>{e.code}</code>\n⏳ Expired: <b>{exp_text}</b>\n\nGunakan tombol di bawah untuk membuka Mini App atau membagikan Experience.')
    if public_domain:
        try:
            try: kb=experience_share(e.code,public_domain,me.username,e.title)
            except TypeError: kb=experience_share(e.code,public_domain,me.username)
        except Exception:
            kb=main(owner(uid))
...
async def text(m:Message,bot:Bot):
    ...
    if step=='title': st['data']['title']=(m.text or '').strip()[:255]; st['step']='intro'; await ask(m,uid)
        ...
        if raw.lower() == '/selesai':
            qs=st['data'].get('questions') or []
            if not qs:
                await m.answer('❌ Belum ada soal. Kirim minimal 1 soal dulu, lalu /selesai.')
                return
            st['step']='media'; await save_draft(uid,st['kind'],'media',json.dumps(st['data'],ensure_ascii=False)); await ask(m,uid); return

        def parse_quiz_batch(text):
            import re
            out=[]
            # Backward-compatible format: question::answer, optionally separated by |.
            if '::' in text:
                ...
                if out: return out
            # Friendly labelled format: pertanyaan: ... / jawaban: ...
            lines=[x.strip() for x in text.splitlines() if x.strip()]
            i=0
            labelled=[]
            while i < len(lines):
                if re.match(r'^pertanyaan\s*:',lines[i],re.I):
                    q=re.sub(r'^pertanyaan\s*:\s*','',lines[i],flags=re.I).strip()
                    if i+1 < len(lines) and re.match(r'^jawaban\s*:',lines[i+1],re.I):
                        a=re.sub(r'^jawaban\s*:\s*','',lines[i+1],flags=re.I).strip(); i+=2
                        if q and a: labelled.append({'question':q,'answer':a}); continue
                i+=1
        ...
        if not batch:
            await m.answer('❌ Format soal belum terbaca. Contoh:\n<code>paus paus apa yang masuk masjid?\npaustadz\n\nsiapa nama gua?\nanse ganteng</code>\n\nAtau format lama: <code>pertanyaan::jawaban</code>.')
            return
        current=st['data'].get('questions') or []
        room=max(0,20-len(current)); added=batch[:room]
        current.extend(added); st['data']['questions']=current[:20]
        await save_draft(uid,st['kind'],'questions',json.dumps(st['data'],ensure_ascii=False))
        msg=f'🧩 Soal diterima ({len(current)}/20).'
        if len(current)>=20: msg+='\n\n⚠️ Maksimal 20 soal tercapai. Ketik /selesai untuk lanjut.'
        else: msg+='\nKirim lagi untuk menambah soal atau ketik /selesai jika sudah.'
        if len(added)<len(batch): msg+='\nBeberapa soal terakhir diabaikan karena batas 20 soal.'
        await m.answer(msg)
        return
    elif step=='media':
