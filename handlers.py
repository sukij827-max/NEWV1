...
async def publish(m,uid):
    ...
    await clear_draft(uid); reset(uid); me=await m.bot.get_me(); public_domain=os.getenv("RAILWAY_PUBLIC_DOMAIN","").strip()
    if public_domain:
        base=public_domain if public_domain.startswith(("http://","https://")) else f"https://{public_domain}"
        link=f"{base.rstrip('/')}/e/{e.code}"
    else:
        link=f"https://t.me/{me.username}?start=exp_{e.code}"
