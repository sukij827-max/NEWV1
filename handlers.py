...
async def ask(m,uid):
    st=STATE[uid]; p={'intro':'📝 <b>Pembuka</b>\nTulis pembuka.','body':'💬 <b>Isi utama</b>\nTulis pesan/isi.','questions':'❓ <b>Pertanyaan</b>\nUntuk Quiz, format: <code>pertanyaan::jawaban</code> dan pisahkan soal dengan <code>|</code>.','media':'📸 <b>Media</b>\nKirim foto/audio. Bisa beberapa foto. /selesai untuk publish atau /skip jika tanpa media.'}
...
async def publish(m,uid):
    ...
    if missing:
        await m.answer('❌ Field wajib kosong: '+', '.join(missing)); return
    e=await create_exp(uid,st['kind'],d['title'],d['intro'],d['body'],json.dumps(d.get('questions',[]),ensure_ascii=False),json.dumps(d.get('media',[]),ensure_ascii=False),d.get('audio'))
    ...
    if public_domain:
        await m.answer(f'🎉 <b>Experience berhasil dibuat!</b>\n\n✨ {e.title}\n🆔 <code>{e.code}</code>\n\nGunakan tombol di bawah untuk membuka Mini App atau membagikan Experience.', reply_markup=experience_share(e.code, public_domain, me.username))
    else:
...
async def audio_upload(m:Message,bot:Bot):
    ...
    await m.answer('🎵 Audio diterima. Kirim foto/audio lagi, /selesai untuk publish, atau /skip.')
...
async def text(m:Message,bot:Bot):
    ...
    if step=='title': st['data']['title']=(m.text or '').strip()[:255]; st['step']='intro'; await ask(m,uid)
        ...
        if m.text and m.text.lower() in {'/skip','/selesai'}:
            ...
            media=list(st['data'].get('media') or [])
            media.append(m.photo[-1].file_id)
            st['data']['media']=media[:20]
            await save_draft(uid,st['kind'],'media',json.dumps(st['data'],ensure_ascii=False))
            await m.answer(f'📸 Foto diterima ({len(media)}/20). Kirim lagi atau /selesai.')
            return
