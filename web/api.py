...
def questions_for(exp):
    ...
    return out[:20]
def expired(e):return bool(e.expires_at and e.expires_at<=datetime.now(timezone.utc))
def theme_for(k):
...
async def miniapp_code(code):return RedirectResponse(f'/miniapp?code={code}',307)
@app.get('/static/{name}')
...
async def audio(code):
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        r=await c.get(f'https://api.telegram.org/bot{token}/getFile',params={'file_id':e.audio_file_id}); fp=(r.json().get('result') or {}).get('file_path') if r.status_code==200 else None
        if not fp:raise HTTPException(404,'Audio not found')
        r=await c.get(f'https://api.telegram.org/file/bot{token}/{fp}')
        if r.status_code!=200:raise HTTPException(404,'Audio not found')
        return Response(r.content,media_type=r.headers.get('content-type','audio/ogg'))
