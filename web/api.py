...
class QuizSubmit(BaseModel): init_data:str=''; participant_id:str=''; answers:list[str]=Field(default_factory=list); duration_ms:int=Field(default=0,ge=0,le=3600000)
...
def experience_payload(exp):
    """Return the structured V4 payload stored in Experience.body.

    V3 records remain valid: anything without the V4 marker is mapped into a
    small legacy payload instead of ever being treated as JSON. This function
    deliberately never raises on malformed user content.
    """
    raw = str(getattr(exp, 'body', '') or '')
    marker = '__GENZ_V4__'
    if raw.startswith(marker):
        candidate = raw[len(marker):].strip()
        try:
    ...
    return {
        'schema_version': 0,
        'category': (str(getattr(exp, 'kind', '')).split(':', 1)[0] if ':' in str(getattr(exp, 'kind', '')) else 'legacy'),
        'style': (str(getattr(exp, 'kind', '')).split(':', 1)[1] if ':' in str(getattr(exp, 'kind', '')) else ''),
        'method': 'legacy',
        'intro': str(getattr(exp, 'intro', '') or ''),
        'body': raw if not raw.startswith(marker) else '',
        'chapters': [],
        'recipient': '',
        'event_date': '',
        'event_location': '',
...
def theme_for(k):
    m={
        'story:midnight':('STORY · MIDNIGHT CINEMA','midnight','A living story, one chapter at a time.'),
        'story:journal':('STORY · DIGITAL JOURNAL','journal','A modern journal made to be explored.'),
        'story:film':('STORY · FILM DIARY','film','A little film made from your memories.'),
        'letter:secret':('LETTER · SECRET','secret','Open slowly. Some words are worth the wait.'),
        'letter:midnight':('LETTER · MIDNIGHT','midnight','A quiet message for someone special.'),
        'letter:postcard':('LETTER · POSTCARD','postcard','A small message from your side of the world.'),
        'event:neon':('EVENT · NEON PARTY','neon','Your event, turned into an experience.'),
        'event:luxury':('EVENT · LUXURY','luxury','A polished invitation with a premium feel.'),
        'event:minimal':('EVENT · MINIMAL','minimal','Everything important. Nothing unnecessary.'),
        'memories:archive':('MEMORIES · PHOTO ARCHIVE','archive','A living archive of moments worth keeping.'),
        'memories:filmroll':('MEMORIES · FILM ROLL','filmroll','Frames, memories, and a little grain.'),
        'memories:wall':('MEMORIES · MEMORY WALL','wall','Your memories, arranged into a living wall.'),
        'play:quizshow':('PLAY · QUIZ SHOW','quizshow','Answer once. Make it count.'),
        'play:whoknows':('PLAY · WHO KNOWS ME?','whoknows','Let’s see how well you really know each other.'),
        'play:random':('PLAY · RANDOM CHALLENGE','random','You got a challenge. Your turn.'),
        'confess':('CONFESSION','romance','Reveal what was left unsaid.'),'loveletter':('LOVE LETTER','romance','A letter made to be opened slowly.'),'anniversary':('ANNIVERSARY','gold','A little timeline of us.'),'appreciation':('APPRECIATION','warm','Something worth saying out loud.'),'friendship':('BEST FRIEND','sunset','For the person who makes ordinary days better.'),'bestiequiz':('BESTIE QUIZ','playful','Let’s see how well you really know each other.'),'farewell':('FAREWELL','dusk','Some moments deserve a gentle goodbye.'),'compat':('COMPATIBILITY','violet','Two people. One little test.'),'birthday':('BIRTHDAY','party','Today deserves a little extra sparkle.'),'graduation':('GRADUATION','blue','A new chapter starts here.'),'invitation':('INVITATION','gold','You are officially invited.'),'countdown':('COUNTDOWN','neon','Every second brings it closer.'),'quiz':('QUIZ','neon','Answer once. Make it count.'),'wyr':('WOULD YOU RATHER','neon','Pick one. No overthinking.'),'tod':('TRUTH OR DARE','danger','Your turn.'),'thisthat':('THIS OR THAT','minimal','One choice says a lot.'),'memory':('MEMORY','film','A place to keep a moment.'),'photostory':('PHOTO STORY','film','Scroll through the story.'),'timeline':('TIMELINE','paper','Little moments, in order.'),'challenge':('CHALLENGE','fire','Ready? Start now.'),'30day':('30-DAY CHALLENGE','fire','One day at a time.'),'randomchallenge':('RANDOM CHALLENGE','glitch','You got this one.')
    }
    return m.get(k,('EXPERIENCE','party','Made just for you.'))
...
async def miniapp():return FileResponse(STATIC/'index.html',media_type='text/html',headers={'Cache-Control':'no-store, no-cache, must-revalidate, max-age=0','Pragma':'no-cache'})
@app.get('/e/{code}')
...
async def miniapp_code(code):
    ...
    return FileResponse(STATIC/'index.html',media_type='text/html',headers={'Cache-Control':'no-store, no-cache, must-revalidate, max-age=0','Pragma':'no-cache'})
@app.get('/static/{name}')
...
async def experience(code):
    async with Session() as s:
        ...
        e.views+=1;await s.commit(); theme=theme_for(e.kind); qs=questions_for(e); payload=experience_payload(e)
        is_quiz=e.kind in QUIZ_KINDS or e.kind.startswith('play:')
        return {'expired':False,'code':e.code,'kind':e.kind,'title':e.title,'intro':e.intro,'body':payload.get('body',e.body),'media':media_ids_for(e),'audio':bool(e.audio_file_id),'quiz': [{'question': q['question']} for q in qs] if is_quiz else [],'plays':e.plays,'views':e.views,'expires_at':e.expires_at.isoformat() if e.expires_at else None,'feedback_enabled':bool(e.feedback_enabled),'theme':{'name':theme[0],'accent':theme[1],'tagline':theme[2]},'experience':payload}
...
def anonymous_participant_id(value):
    # Keep shared-link participants stable per browser while staying inside a
    # signed BIGINT range and never colliding with normal Telegram IDs.
...
async def submit_quiz(code,payload:QuizSubmit):
    ...
    if u:
        uid=int(u['id']); name=(u.get('first_name') or u.get('username') or 'Anonymous')[:80]
    elif payload.participant_id and len(payload.participant_id)<=200:
        uid=anonymous_participant_id(payload.participant_id); name='Anonymous'
    else:
        raise HTTPException(400,'Unable to identify this participant. Open the shared link again.')
    async with Session() as s:
        e=await s.scalar(select(Experience).where(Experience.code==code,Experience.published.is_(True)))
        if not e:raise HTTPException(404,'Expression not found')
        if expired(e):raise HTTPException(410,'This Expression has expired.')
        ...
        existing=await s.scalar(select(QuizAttempt).where(QuizAttempt.expression_code==code,QuizAttempt.telegram_id==uid))
        if existing:
            rank=(await s.scalar(select(func.count()).where(QuizAttempt.expression_code==code,QuizAttempt.score>existing.score))) or 0
            return {'already_submitted':True,'score':existing.score,'total':existing.total,'rank':rank+1,'anonymous':not bool(u)}
        answers=payload.answers[:len(qs)]
        score=sum(1 for q,a in zip(qs,answers) if q['answer'] and a.strip().casefold()==q['answer'].strip().casefold())
        ...
        try:await s.commit()
            ...
            return {'already_submitted':True,'score':existing.score,'total':existing.total,'rank':1,'anonymous':not bool(u)}
        rank=(await s.scalar(select(func.count()).where(QuizAttempt.expression_code==code,QuizAttempt.score>score))) or 0
        return {'already_submitted':False,'score':score,'total':len(qs),'rank':rank+1,'anonymous':not bool(u)}
@app.post('/api/experience/{code}/message')
async def owner_message(code,payload:OwnerMessageIn,request:Request):
    # A shared Experience URL can be opened in Telegram's in-app browser (or a
    # normal browser). That context does NOT provide Telegram WebApp initData.
    # Previously we rejected those messages with 401, making "Send to creator"
    # appear broken even though the Experience itself opened correctly.
    #
    # If valid initData exists, keep the sender identity. Otherwise allow an
    # anonymous message and still route it to the Experience creator.
    u=verify_init_data(payload.init_data)
    e=await active_exp(code)
    if not e:raise HTTPException(404,'Expression not found')
    if expired(e):raise HTTPException(410,'This Expression has expired.')
    ...
    name=((u.get('first_name') or u.get('username')) if u else None) or 'Anonymous'
    name=str(name)[:255]
    now=datetime.now(timezone.utc)
    # Rate-limit verified Telegram users by Telegram ID. Anonymous shared-web
    # visitors are not blocked by a single global sender_id=0 bucket.
    if uid:
        async with Session() as s:
    ...
    if token:
        import httpx
        sender_line=f'👤 {html.escape(name)}' if uid else '👤 Anonymous (shared link)'
        id_line=f'\n🆔 <code>{uid}</code>' if uid else ''
        msg=f'💌 <b>Pesan baru dari Experience</b>\n\n🌐 <b>{html.escape(e.title)}</b>\n🆔 <code>{html.escape(code)}</code>\n{sender_line}{id_line}\n\n💬 {html.escape(text)}'
        try:
            async with httpx.AsyncClient(timeout=15) as c:
                r=await c.post(f'https://api.telegram.org/bot{token}/sendMessage',json={'chat_id':e.owner_id,'text':msg,'parse_mode':'HTML'})
                delivered=bool(r.status_code==200 and r.json().get('ok'))
        except Exception:delivered=False
    async with Session() as s:
        s.add(OwnerMessage(expression_code=code,owner_id=e.owner_id,sender_id=uid,sender_name=name,text=text,delivered=delivered))
        await s.commit()
    if not delivered:raise HTTPException(502,'Message saved but could not be delivered to the creator right now.')
    return {'ok':True,'anonymous':not bool(u)}
