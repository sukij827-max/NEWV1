...
def Session(): return async_sessionmaker(get_engine(), expire_on_commit=False)()
...
async def init_db():
    async with get_engine().begin() as c:
        ...
        def migrate(sync_conn):
            ...
            def cols(table): return {x['name'] for x in insp.get_columns(table)} if insp.has_table(table) else set()
            ecols=cols('experiences')
            if 'expires_at' not in ecols: sync_conn.execute(text('ALTER TABLE experiences ADD COLUMN expires_at DATETIME'))
            if 'feedback_enabled' not in ecols: sync_conn.execute(text('ALTER TABLE experiences ADD COLUMN feedback_enabled BOOLEAN DEFAULT 1'))
            if not insp.has_table('owner_messages'):
                OwnerMessage.__table__.create(sync_conn)
            # Repair old duplicate quiz submissions before creating the uniqueness guard.
            if insp.has_table('quiz_attempts'):
                sync_conn.execute(text('''DELETE FROM quiz_attempts WHERE id NOT IN (SELECT MAX(id) FROM quiz_attempts GROUP BY expression_code, telegram_id)'''))
                try: sync_conn.execute(text('CREATE UNIQUE INDEX IF NOT EXISTS uq_quiz_expression_user_idx ON quiz_attempts(expression_code, telegram_id)'))
