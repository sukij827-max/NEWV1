...
async def init_db():
    async with get_engine().begin() as c:
        ...
        def migrate(conn):
            ...
            # V3 columns added to an existing V2 PostgreSQL table.
            experience_columns={
                'audio_file_id':'TEXT',
            ...
            if 'expires_at' not in cols:
                conn.execute(text('ALTER TABLE experiences ADD COLUMN expires_at TIMESTAMP WITH TIME ZONE' if dialect=='postgresql' else 'ALTER TABLE experiences ADD COLUMN expires_at DATETIME'))
            if 'feedback_enabled' not in cols:
                conn.execute(text('ALTER TABLE experiences ADD COLUMN feedback_enabled BOOLEAN DEFAULT TRUE' if dialect=='postgresql' else 'ALTER TABLE experiences ADD COLUMN feedback_enabled BOOLEAN DEFAULT 1'))
            if dialect=='postgresql':
                # Existing V2 rows may have NULL after adding the column.
                conn.execute(text('UPDATE experiences SET feedback_enabled = TRUE WHERE feedback_enabled IS NULL'))
...
async def create_exp(uid,kind,title,intro,body,questions,media,audio=None,expires_at=None):
    ...
    async with Session() as s:
        ...
        e=Experience(code=code,owner_id=uid,kind=kind,title=title,intro=intro,body=body,questions=questions,media_file_id=media,audio_file_id=audio,expires_at=expires_at); s.add(e); await s.commit(); return e
