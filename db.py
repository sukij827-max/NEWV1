...
from sqlalchemy import BigInteger, Boolean, DateTime, Integer, String, Text, select, func, update
...
async def create_exp(uid,kind,title,intro,body,questions,media):
    ...
    async with Session() as s:
        ...
        e=Experience(code=code,owner_id=uid,kind=kind,title=title,intro=intro,body=body,questions=questions,media_file_id=media); s.add(e); await s.commit(); return e
