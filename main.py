...
def main_menu():
    ...
    rows.append([InlineKeyboardButton(text="🎮 Buat Quiz",callback_data="quiz:start")])
    rows.append([InlineKeyboardButton(text="📁 Experience Saya",callback_data="mine")])
...
async def finish_creation(m,state):
    ...
    try:
        ...
        await progress.edit_text(
            f"✨ <b>Expression berhasil dibuat!</b>\n\n"
            f"✨ <a href='{PUBLIC_BASE_URL}/miniapp/{wid}'>Buka Experience</a>\n"
            f"🆔 <code>{wid}</code>\n\n"
            f"📱 Dibuka melalui Telegram Mini App / server Railway kamu.",
            parse_mode="HTML"
        )
    except Exception as e:
        await progress.edit_text(
...
async def mine(c):
    ...
    kb=[]
    for x in items[:30]:
        kb.append([InlineKeyboardButton(text=f"🌐 {x['id']} · {x['template_id']}",url=f"{PUBLIC_BASE_URL}/miniapp/{x['id']}")])
...
async def quiz_start(c,state):
    if not await ensure_access(c): return
    await state.clear(); await state.set_state(QuizForm.title)
    await c.message.answer("🎮 <b>Buat Quiz</b>\n\nKirim judul quiz.",parse_mode="HTML")
    await c.answer()

@dp.message(Command("quiz"))
async def quiz_command(m,state):
    if not await ensure_access(m): return
    await state.clear(); await state.set_state(QuizForm.title)
    await m.answer("🎮 <b>Buat Quiz</b>\n\nKirim judul quiz.",parse_mode="HTML")

@dp.message(StateFilter(QuizForm.title),F.text)
async def quiz_title(m,state):
    title=m.text.strip()[:120]
    if not title: await m.answer("❌ Judul tidak boleh kosong."); return
    await state.update_data(quiz_title=title,quiz_questions=[])
    await state.set_state(QuizForm.questions)
    await m.answer("Kirim pertanyaan satu per satu dengan format:\n<code>Pertanyaan | Jawaban benar | Opsi salah | Opsi salah</code>\n\nContoh: <code>2+2? | 4 | 3 | 5</code>\n\nKetik /donequiz jika selesai.",parse_mode="HTML")

@dp.message(Command("donequiz"),StateFilter(QuizForm.questions))
async def quiz_done(m,state):
    d=await state.get_data(); qs=d.get("quiz_questions") or []
    if not qs: await m.answer("❌ Tambahkan minimal 1 pertanyaan."); return
    qid=create_quiz(m.from_user.id,d["quiz_title"],qs)
    wid=create_website(m.from_user.id,"quiz","quickquiz",{"title":d["quiz_title"],"intro":"Answer honestly. Let's see your score.","quiz_json":json.dumps(qs,ensure_ascii=False)})
    build_all()
    await state.clear()
    await m.answer(f"🏆 <b>Quiz siap!</b>\n\n🔗 <a href='{PUBLIC_BASE_URL}/miniapp/{wid}'>Buka & Share Mini App</a>\n🆔 Quiz: <code>{qid}</code>\n\nLeaderboard tersimpan per quiz.",parse_mode="HTML")

@dp.message(StateFilter(QuizForm.questions),F.text)
async def quiz_question(m,state):
    if m.text.startswith("/"): return
    parts=[x.strip() for x in m.text.split("|")]
    if len(parts)<3: await m.answer("❌ Format salah. Gunakan: Pertanyaan | Jawaban benar | Opsi salah ..."); return
    q,correct,*wrong=parts
    options=[correct]+wrong
    import random; random.shuffle(options)
    answer=options.index(correct)
    d=await state.get_data(); qs=list(d.get("quiz_questions") or [])
    qs.append({"q":q[:300],"options":options[:6],"answer":answer})
    await state.update_data(quiz_questions=qs)
    await m.answer(f"✅ Pertanyaan #{len(qs)} tersimpan. Kirim pertanyaan berikutnya atau /donequiz.")
...
async def adm_redeploy(c):
    ...
    try:build_all();await c.message.answer("✅ Semua Expression berhasil dibangun ulang di server Mini App.")
